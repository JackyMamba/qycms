...
<div class="procate-index">
	...
	<table class="table table-striped">
		...
		<tr level="<?=$v['nested_level']?>" sortable-group-id="<?=$v['pid']?>" item-id="<?=$v['id']?>" parents="<?=str_replace(',', ' ', $v['pids'])?>">
			...
			<td><?=$v['name']?></td>
			<td><?=$v['status'] == Procate::STATUS_ACTIVE ? '<span class="text-green">发布</span>' : ($v['status'] == Procate::STATUS_EDIT ? '<span class="">编辑中</span>' : ($v['status'] == Procate::STATUS_HIDE ? '<span class="text-muted">隐藏</span>' : ''))?></td>
			...
			<td class="action-btn-td"><?=Html::a('<i class="fa fa-pencil-square-o"></i>', [
                    ...
                    'aria-label' => '修改',
                ]) . Html::a($v['status'] == Procate::STATUS_HIDE ? '<i class="fa fa-fw fa-toggle-off text-muted"></i>' : '<i class="fa fa-fw fa-toggle-on text-green"></i>', [
                    'hide',
                    'id' => $v['id']
                ], [
